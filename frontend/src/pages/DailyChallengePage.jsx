...
export default function DailyChallengePage() {
  ...
  useEffect(() => {
    ...
    fetch(API + "/today").then(r => r.json()).then(data => {
      setDate(data.date); setClues([data.firstClue]); setTotalClues(data.totalClues); setMaxGuesses(data.maxGuesses || 6); setPhase("playing");
    }).catch(() => setPhase("error"));
  }, []);
  const getNextClue = async () => {
    if (clueIndex >= totalClues) return;
    const r = await fetch(API + "/clue", { method: "POST", headers: {"Content-Type":"application/json"}, body: JSON.stringify({ clueIndex }) });
    const data = await r.json();
    if (data.clue) { setClues(c => [...c, data.clue]); setClueIndex((data.clueIndex || clueIndex) + 1); if (data.allCluesShown) setClueIndex(totalClues); }
  };
  const submitGuess = async () => {
    ...
    const isGameOver = data.correct || nextGuessNum > maxGuesses;
    if (isGameOver) {
      const rd = {
        correct: data.correct,
        answer: data.answer,
        score: data.correct ? Math.max(100, 1000 - (clues.length * 100) - ((guessNum - 1) * 150)) : 0,
      ...
      setWrongGuess(true);
      setTimeout(() => setWrongGuess(false), 800);
    }
    setGuess("");
  };
    ...
    <div className="game-page">
      ...
      {phase === "playing" && (<div className="game-card" style={{maxWidth:"640px",margin:"0 auto"}}>
        <div className="game-card-header"><div className="game-status">
          ...
          <div className="game-stat"><div className="game-stat-val">{maxGuesses - guessNum + 1}</div><div className="game-stat-label">Guesses Left</div></div>
        </div>
        <div className="guess-dots">{Array.from({length:maxGuesses}).map((_,i) => (<div key={i} className={"guess-dot"+(i<guessNum-1?" used":i===guessNum-1?" active":"")}/>))}</div>
        </div>
        ...
        <div className="guess-area">
          <input className={"guess-input" + (wrongGuess ? " wrong-flash" : "")} value={guess} onChange={e=>setGuess(e.target.value)} onKeyDown={e=>e.key==="Enter"&&submitGuess()} placeholder="Type player name..." autoFocus/>
